...
# Plots the experimental Cp plus the Cp for each limiter for grid G2
# 
...
# Experimental run
file_exp = 'CP_Gregory_expdata_10.dat'
exp_data = pd.read_csv(file_exp,delim_whitespace=True,skiprows=4)
x_exp = exp_data['x/c']
cp_exp = exp_data.cp

# CFD runs
filename0 = 'surface_flowG2_Lim0.1_AoA10.csv'
filename1 = 'surface_flowG2_Lim1_AoA10.csv'
filename2 = 'surface_flowG2_Lim10_AoA10.csv'
...
ax.plot(x0,cp0,label='$lim=0.1$',color='r',lw=2)
ax.plot(x1,cp1,label='$lim=1$',color='b',lw=2)
ax.plot(x2,cp2,label='$lim=10$',color='g',lw=2)
ax.plot(x_exp,cp_exp,label='$exp$',color='k',linestyle='o',marker='o',fillstyle='none',markersize=8)
ax.legend(loc='best',fancybox=True)
ax.set_yticks([-6,1])
ax.set_xticks([0,1])
ax.set_ylim(-7.045,2.045)
ax.set_xlim(-0.05,1.05)

ax.set_ylim(ax.get_ylim()[::-1]) #flip the y axis.
fig.savefig('Cp_vs_Lim_G2_deg10.pdf')
plt.show()
